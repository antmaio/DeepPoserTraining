...
def parse(opt_path, args=None, is_train=True):
    ...
    if args is not None:
        ...
        assert 'task' in args.__dict__, "please provide task name in args"
        opt['task'] = args.task #if provided, override task from json
        opt['datasets']['train']['yolo_model'] = args.yolo_model
        opt['datasets']['test']['yolo_model'] = args.yolo_model

        if 'protocol' in args.__dict__ and args.protocol is not None:
            
            if args.yolo_model is not None:
                opt['datasets']['train']['dataset_type'] = 'amass_yolo_p' + args.protocol
                opt['datasets']['test']['dataset_type'] = 'amass_yolo_p' + args.protocol

                if args.protocol in ['1', '2']:
                    opt['datasets']['train']['dataroot'] = f"./data/{args.yolo_model}_protocol_1"
                    opt['datasets']['test']['dataroot'] = f"./data/{args.yolo_model}_protocol_1"
                else:
                    opt['datasets']['train']['dataroot'] = f"./data/{args.yolo_model}_protocol_3"
                    opt['datasets']['test']['dataroot'] = f"./data/{args.yolo_model}_protocol_3"
