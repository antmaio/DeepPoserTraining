...
def matrix_to_angle_axis(matrix: torch.Tensor) -> torch.Tensor:
    """
    Convert rotation matrices to axis/angle with improved numerical stability.
    """
    if matrix.size(-1) != 3 or matrix.size(-2) != 3:
        raise ValueError(f"Invalid rotation matrix shape {matrix.shape}.")

    # Compute the anti-symmetric component
    omegas = torch.stack([
    ...
    eps = EPSILON
    safe_norms = torch.where(norms < eps, torch.ones_like(norms), norms)
    
    # Handle small angle case more carefully
    small_angle = safe_norms.squeeze(-1) < eps  # Remove last dimension for masking
    
    # Standard case (angle not near 0 or π)
    axis_angles = torch.empty_like(omegas)
    
    # Create pi tensor with same device and dtype as angles
    pi_tensor = torch.tensor(torch.pi, device=angles.device, dtype=angles.dtype)
    
    # For angles not near 0 or π, use the standard formula
    not_special = ~small_angle & ~angles.isclose(pi_tensor, rtol=1e-4).squeeze(-1)
    if not_special.any():
        axis_angles[not_special] = (angles[not_special] / safe_norms[not_special]) * omegas[not_special]
    
    # Handle angle ≈ π case (more robust)
    near_pi = (angles - pi_tensor).abs().squeeze(-1) < 1e-4
    if near_pi.any():
        # More stable computation for π case
        # Use the formula: n = sqrt(diag(R + I)/2)
        R_plus_I = matrix[near_pi] + torch.eye(3, device=matrix.device, dtype=matrix.dtype)
        diag = torch.diagonal(R_plus_I, dim1=-2, dim2=-1)
        n = torch.sqrt(torch.clamp(diag / 2, min=eps))
        
        # Ensure correct signs by checking off-diagonal elements
        signs = torch.sign(R_plus_I[..., 2, 1] - R_plus_I[..., 1, 2])
        n = n * signs.unsqueeze(-1)
        
        axis_angles[near_pi] = angles[near_pi] * F.normalize(n, p=2, dim=-1)
    
    # Handle very small angles (near identity)
    if small_angle.any():
        axis_angles[small_angle] = 0.5 * omegas[small_angle]  # First order approximation
...
def two_axis_to_matrix(two_axis: torch.Tensor) -> torch.Tensor:
    ...
    *leading_shape, last_dim = two_axis.shape
    ...
    in1 = two_axis[..., 0:3]
    col1 = in1 / (torch.linalg.norm(in1, dim=-1, keepdim=True)  + EPSILON)
    matrix[..., 0] = col1

    in2 = two_axis[..., 3:6]
    col2 = in2 - torch.linalg.vecdot(in2, col1)[..., None] * col1
    col2 = col2 / (torch.linalg.norm(col2, dim=-1, keepdim=True) + EPSILON)
    matrix[..., 1] = col2
...
# Delta 
def compute_rotation_delta(rot: torch.Tensor):
    """
    Compute the frame-wise delta (to a previous frame) of a rotation matrix 'rot'.
    First frame delta is handled using a zero jerk approximation.
    @param rot: tensor of shape (..., num_frames, 3, 3).
    @return: tensor with same shape as 'rot'.
    """
    assert len(rot.shape) >= 3
    assert rot.shape[-3] >= 3
    assert rot.shape[-2:] == (3, 3)
    # Delta is from previous frame (not to the next frame) [d_i = r_i @ (r_{i-1})^{-1}]
    rot_delta_rest = rot[..., 1:, :, :] @ torch.linalg.inv(rot[..., :-1, :, :])
    # zero jerk approximation of first frame [d0 := (d2 @ d1^{-1})^{-1} @ d1 = d1 @ d2^{-1} @ d1]
    d1 = rot_delta_rest[..., 0:1, :, :]
    d2 = rot_delta_rest[..., 1:2, :, :]
    rot_delta_first = d1 @ torch.linalg.inv(d2) @ d1
    rot_delta = torch.cat([rot_delta_first, rot_delta_rest], dim=-3)
    return rot_delta


def compute_positional_delta(pos: torch.Tensor):
    """
    Compute the frame-wise delta (to a previous frame) of a vector 'pos'.
    First frame delta is handled using a zero jerk approximation.
    @param pos: tensor of shape (..., num_frames, dim).
    @return: tensor with same shape as 'pos'.
    """
    assert len(pos.shape) >= 2
    assert pos.shape[-2] >= 3
    # Delta is from previous frame (not to the next frame) [d_i = p_i - p_{i-1}]
    pos_delta_rest = pos[..., 1:, :] - pos[..., :-1, :]
    # zero jerk approximation of first frame [d0 := d1 - (d2 - d1) = 2 * d1 - d2]
    d1 = pos_delta_rest[..., 0:1, :]
    d2 = pos_delta_rest[..., 1:2, :]
    pos_delta_first = (2 * d1) - d2
    pos_delta = torch.cat([pos_delta_first, pos_delta_rest], dim=-2)
    return pos_delta
