"""Multi-view 3D triangulation pipeline.

This module loads project configuration from a TOML file, resolves the
appropriate virtual-camera directories from the configured camera paths,
collects preprocessed YOLO keypoint files from the dataset output tree, and
reconstructs 3D joint positions from the selected camera pairs.

The script is designed to operate on data generated by the preparation stage,
where each dataset split is stored under a folder such as
``{data_path.output_dir}/{yolo_model}_protocol_{protocol}/...``.

Typical usage:
    python pose_lifter/triang.py configs/prepare_data.toml
"""
...
class Config:
    """Container for triangulation runtime configuration.

    Attributes:
        dataset_type: Dataset family name used to select the correct
            triangulation branch (for example ``amass_p1``).
        dataroot: Root directory containing the preprocessed YOLO PKL files.
        output_dir: Relative directory where reconstructed 3D outputs are saved.
        camera_paths: Mapping of protocol identifiers to the configured virtual
            camera directories.
    """
    dataset_type: str
    dataroot: str
...
def _load_config(args: argparse.Namespace) -> Config:
    ...
    pose_cfg = toml.get('pose_lifter', {})

    protocol = pr_cfg.get('protocol', 1)
    yolo_model = yolo_cfg.get('model', 'yolov8n-pose')
    cfg.dataset_type = {
        1: 'amass_p1',
        2: 'amass_p2',
        3: 'amass_p3',
    }.get(protocol, 'amass_p1')
    cfg.dataroot = os.path.join(
        dat_cfg.get('output_dir', './data/keypoints/'),
        f'{yolo_model}_protocol_{protocol}',
    )
    cfg.output_dir = pose_cfg.get('relative_output_dir', 'triang')
    cfg.camera_paths = load_camera_paths(toml_data=toml)

    return cfg


def _get_dataset_camera_path(cfg: Config) -> str:
    """Map the selected dataset type to the configured camera directory."""
    dataset_to_protocol = {
        'amass_p1': 1,
        'amass_p2': 2,
        'amass_p3': 3,
    }
    protocol = dataset_to_protocol.get(cfg.dataset_type, 1)
    return cfg.camera_paths[protocol]


def log_config(cfg: Config):
    """Log the configuration state in cyan."""
    CYAN = "\033[36m"
    RESET = "\033[0m"
...
def _build_filename_list(dataset_type: str, phase: str, dataroot: str) -> List[str]:
    ...
    if dataset_type == 'amass_p1':
        pattern = os.path.join(dataroot, '*', phase, 'preprocessed', '*.pkl')
...
def main():
    """Run the complete triangulation pipeline from the TOML config.

    Loads the configuration file, resolves camera paths and dataset root,
    discovers all relevant preprocessed keypoint files, triangulates each joint
    across the selected camera pairs, and saves the resulting 3D arrays as
    ``.npz`` files alongside the input data tree.
    """
    ...
    camera_path     = _get_dataset_camera_path(cfg)
